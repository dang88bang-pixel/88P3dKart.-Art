...
### 5.9 Network3D- & Taktik-Aktionen (docs/NETWORK3D.md, docs/TACTICAL.md)

| ID | Name | Trigger | Verhalten | Feedback |
|----|------|---------|-----------|----------|
| A-42 | Topologie anzeigen | Toggle `🌐 Topologie` / Layer-Chip | Nodes (Typ-/Statusfarben) + Spline-Edges (Auslastungsfarben) + Flow-Partikel im 3D-View | sofort; kritische Nodes pulsieren |
| A-43 | What-If-Failover | Node-Klick → „Ausfall simulieren" | `POST /api/v1/network/simulate` → betroffene/reroutete/unerreichbare Flows hervorheben | HUD-Ergebniszeile |
| A-44 | Time Machine | Slider im Topologie-Panel | Snapshot-Replay aus `GET /api/v1/network/history` | Zeitachse + Status |
...
8. **Kein Regression:** bestehende 122 Edge-Agent-Tests + 157 JVM-Tests grün, `node --check` sauber, kein UI-Code im Render-Thread.
